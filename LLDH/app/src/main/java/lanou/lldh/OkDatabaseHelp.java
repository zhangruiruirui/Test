...
//            + COLUMN_ID + " INTEGER primary key autoincrement,"
    ...
    public List<OkDownloadRequest> execQuery(String key, String value) {
        SQLiteDatabase db = getWritableDatabase();

        List<OkDownloadRequest> requestList = new ArrayList<>();
        ...
        if (cursor.getCount() != 0) {
            if (cursor.moveToFirst()) {
                do {
                    OkDownloadRequest okDownloadRequest = new OkDownloadRequest();
...
//        db.close();
    ...
    public List<OkDownloadRequest> execQueryAll() {
        SQLiteDatabase db = getWritableDatabase();
